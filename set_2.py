...
def shift_by_letter(letter, letter_shift):
    ...
    if letter == " ":
        return " "
    shift = (((ord(letter) - ord("A")) + (ord(letter_shift) - ord("A"))) % 26)
...
def vigenere_cipher(message, key):
    ...
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    result = []
    keystream = (key * -(-len(message) // len(key)))[:len(message)] 

    for i in range(len(message)):
        message_char = message[i]
        keystream_char = keystream[i]

        if message_char == ' ':
            result.append(' ')
        else:
            result.append(chr((ord(message_char) + ord(keystream_char) - 2 * ord('A')) % 26 + ord('A')))
